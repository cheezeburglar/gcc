/* basically do optinfo-emit-json
...
//
//void
//tree_json_writer::write() const 
//{
//
//}
//
//json::object * //!!!
//tree_json_writer::tree_to_json(tree t)
//{
//
//}
//
//json::object *
//tree_json_writer::node_to_json(tree t)
//{
//
//}
//
//json::object *
//tree_json_writer::tree_insert_node(tree t)
//{
//  parent_node->append(node_to_json(
//}
//
/*==============================================================================*/
...
queue_and_dump_index (dump_info_p di, const char *field, const_tree t, int flags)
{
  ...
  /* This should be child node */
  child.set_integer("index", index);
  child.set_string ("field", field);
...
queue_and_dump_type (dump_info_p di, const_tree t)
{
  ...
  child.set_integer("index", index);
  child.set_string ("type", TREE_TYPE(t));
  di->tree_json.append(child)
}
...
/* IN PROGRESS - MAKE JSON TREE, write to stream*/
...
dequeue_and_dump (dump_info_p di)
{
  ...
  //*** MAKE PARENT HERE?
  /* Set node index.  */
  json_tree.set("index", index);
  /* And the type of node this is.  */
  if (dni->binfo_p)
    code_name = "binfo";
  else
    code_name = get_tree_code_name (TREE_CODE (t));
  json_tree.set("tree_code", code_name);
...
void
dump_json_tree (const_tree t, dump_flags_t flags, FILE *stream)
{
  struct dump_info di;
  dump_queue_p dq;
...
/*TEST
  ...
  while (di.queue)
    /* CATCH HERE! for origins of dump node */
    dequeue_and_dump (&di);
...
/* IN PROGRESS - needs to write at TJW at some point. or ameliorate dumps so they add to tjw*/

static void
dequeue_and_dump (dump_info_p di, tree_json_writer tjw)
{
  dump_queue_p dq;
  splay_tree_node stn;
  dump_node_info_p dni;
...
/*AH?*/
  ...
  if (IS_EXPR_CODE_CLASS (code_class))
	...
	{
	case tcc_unary:
          json_tree.set("op 0", TREE_OPERAND(t, 0));
//	  dump_child ("op 0", TREE_OPERAND (t, 0));
	  break;
...
dump_json_tree (const_tree t, dump_flags_t flags, FILE *stream)
{
  ...
  /* Initialize the dump-information structure.  */
  di.stream = stream;
  di.json_writer = new tree_json_writer(); 
  di.index = 0;
  di.column = 0;
  di.queue = 0;
  di.queue_end = 0;
