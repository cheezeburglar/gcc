...
extern void lhd_overwrite_decl_assembler_name(tree, tree);
extern tree_node* lhd_type_promotes_to(tree);
...
extern tree_node* lhd_pass_through_t(tree);
extern tree_node* lhd_do_nothing_iii_return_null_tree(int, int, int);
extern void lhd_do_nothing_f(function*);
extern tree_node* lhd_return_null_tree(tree);
extern tree_node* lhd_return_null_const_tree(const_tree);
extern int lhd_gimplify_expr(tree_node**, gimple**, gimple**);
extern size_t lhd_tree_size(tree_code);
extern bool lhd_decl_ok_for_sibcall(const_tree);
extern void lhd_initialize_diagnostics(diagnostic_context*);
extern bool lhd_handle_option(size_t, const char*, long int, int, location_t, const cl_option_handlers*);
extern tree_node* lhd_simulate_record_decl(location_t, const char*, array_slice<tree_node* const>);
extern tree_node* lhd_type_for_size(unsigned int, int);
extern long int lhd_to_target_charset(long int);
extern tree_node* lhd_expr_to_decl(tree, bool*, bool*);
extern omp_clause_default_kind lhd_omp_predetermined_sharing(tree);
extern omp_clause_defaultmap_kind lhd_omp_predetermined_mapping(tree);
extern tree_node* lhd_omp_assignment(tree, tree, tree);
extern void lhd_omp_finish_clause(tree, gimple**, bool);
extern tree_node* lhd_omp_array_size(tree, gimple**);
extern bool lhd_omp_deep_mapping_p(const gimple*, tree);
extern tree_node* lhd_omp_deep_mapping_cnt(const gimple*, tree, gimple**);
extern void lhd_omp_deep_mapping(const gimple*, tree, long unsigned int, tree, tree, tree, tree, tree, gimple**);
extern tree_node* lhd_omp_mapper_lookup(tree, tree);
extern tree_node* lhd_omp_extract_mapper_directive(tree);
extern tree_node* lhd_omp_map_array_section(location_t, tree);
extern bool lhd_omp_scalar_p(tree, bool);
extern tree_node** lhd_omp_get_decl_init(tree);
extern void lhd_omp_finish_decl_inits();
extern void lhd_omp_firstprivatize_type_sizes(gimplify_omp_ctx*, tree);
extern tree_node* lhd_builtin_function(tree);
extern void lhd_begin_section(const char*);
extern void lhd_append_data(const void*, size_t, void*);
extern void lhd_end_section();
extern tree_node* lhd_enum_underlying_base_type(const_tree);
extern const char* lhd_get_substring_location(const substring_loc&, location_t*);
extern int lhd_decl_dwarf_attribute(const_tree, int);
extern int lhd_type_dwarf_attribute(const_tree, int);
extern tree_node* lhd_unit_size_without_reusable_padding(tree);
extern void lhd_finalize_early_debug();
