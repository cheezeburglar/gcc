...
void lhd_overwrite_decl_assembler_name(tree, tree);
tree_node* lhd_type_promotes_to(tree);
...
const char* lhd_decl_printable_name(tree, int);
const char* lhd_dwarf_name(tree, int);
...
tree_node* lhd_pass_through_t(tree);
tree_node* lhd_do_nothing_iii_return_null_tree(int, int, int);
void lhd_do_nothing_f(function*);
tree_node* lhd_return_null_tree(tree);
tree_node* lhd_return_null_const_tree(const_tree);
int lhd_gimplify_expr(tree_node**, gimple**, gimple**);
size_t lhd_tree_size(tree_code);
bool lhd_decl_ok_for_sibcall(const_tree);
void lhd_initialize_diagnostics(diagnostic_context*);
bool lhd_handle_option(size_t, const char*, long int, int, location_t, const cl_option_handlers*);
tree_node* lhd_simulate_record_decl(location_t, const char*, array_slice<tree_node* const>);
tree_node* lhd_type_for_size(unsigned int, int);
long int lhd_to_target_charset(long int);
tree_node* lhd_expr_to_decl(tree, bool*, bool*);
omp_clause_default_kind lhd_omp_predetermined_sharing(tree);
omp_clause_defaultmap_kind lhd_omp_predetermined_mapping(tree);
tree_node* lhd_omp_assignment(tree, tree, tree);
void lhd_omp_finish_clause(tree, gimple**, bool);
tree_node* lhd_omp_array_size(tree, gimple**);
bool lhd_omp_deep_mapping_p(const gimple*, tree);
tree_node* lhd_omp_deep_mapping_cnt(const gimple*, tree, gimple**);
void lhd_omp_deep_mapping(const gimple*, tree, long unsigned int, tree, tree, tree, tree, tree, gimple**);
tree_node* lhd_omp_mapper_lookup(tree, tree);
tree_node* lhd_omp_extract_mapper_directive(tree);
tree_node* lhd_omp_map_array_section(location_t, tree);
bool lhd_omp_scalar_p(tree, bool);
tree_node** lhd_omp_get_decl_init(tree);
void lhd_omp_finish_decl_inits();
void lhd_omp_firstprivatize_type_sizes(gimplify_omp_ctx*, tree);
tree_node* lhd_builtin_function(tree);
void lhd_begin_section(const char*);
void lhd_append_data(const void*, size_t, void*);
void lhd_end_section();
tree_node* lhd_enum_underlying_base_type(const_tree);
const char* lhd_get_substring_location(const substring_loc&, location_t*);
int lhd_decl_dwarf_attribute(const_tree, int);
int lhd_type_dwarf_attribute(const_tree, int);
tree_node* lhd_unit_size_without_reusable_padding(tree);
void lhd_finalize_early_debug();
