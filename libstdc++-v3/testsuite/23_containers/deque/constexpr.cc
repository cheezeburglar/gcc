...
constexpr bool ctor_tests()
{
  ...
  Alloc<int> alloc (6);
  std::deque<int> dq2 (alloc);
  std::deque<int> dq3 (4, alloc);
  VERIFY( dq3.front() == 0 );
  VERIFY( dq3.size() == 4 );
  std::deque<int> dq4 (4, 5, alloc);
  VERIFY( dq4.front() == 5 );
  VERIFY( dq4.size() == 4 );
  ...
  std::deque<int> dq9 (dq1, alloc);
  std::deque<int> dq10 (std::move(dq1), alloc);
  VERIFY ( dq9 == dq10 );
  std::deque<int> dq11 (rg, alloc);
  VERIFY( dq11 == dq5 );
