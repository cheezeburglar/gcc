...
constexpr bool ctor_tests()
{
  ...
  auto pq13 = std::priority_queue<int>(std::from_range_t, std::ranges::views::iota(0, 7), std::less<int>());
  VERIFY(pq13.size() == 7);
...
constexpr bool alloc_aware_ctor_tests()
{
  ...
  auto pq23 = std::priority_queue<int>(std::from_range_t, std::ranges::views::iota(0, 7), alloc);
  VERIFY(pq23.size() == 7);

  auto pq24 = std::priority_queue<int>(std::from_range_t, std::ranges::views::iota(0, 7), std::less<int>(), alloc);
  VERIFY(pq24.size() == 7);
